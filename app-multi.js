...
/** Rendu carte readonly — diff intelligent pour éviter de recréer tout le DOM */
function renderCardsTo(holder, cards){
  if (!holder) return;
  const list = cards || [];
  // Si même nombre et mêmes ids dans le même ordre → mise à jour légère des classes seulement
  const existing = Array.from(holder.querySelectorAll('.card'));
  const sameIds = existing.length === list.length && list.every((c, i) => existing[i]?.dataset.cardId === c.id);
  if (sameIds) {
    list.forEach((c, i) => {
      const el = existing[i];
      el.classList.toggle('tapped',  !!c.tapped);
      el.classList.toggle('phased',  !!c.phased);
      el.classList.toggle('has-badge', !!c.hasBadge);
    });
    return;
  }
  // Sinon reconstruire
  ...
  list.forEach(c => {
    const el = createCardEl(
      { id: c.id, name: c.name, type: c.type, imageSmall: c.imageSmall || null, imageNormal: c.imageNormal || null, hasBadge: !!c.hasBadge, badgeItems: c.badgeItems || [], cardStats: c.cardStats || null },
...
function renderNamesWithPreview(holder, cards){
  ...
  if (count > 0) {
    ...
    badge.style.cssText = 'font-size:12px;color:#9aa3b2;padding:4px 6px;';
...
function setupMultiplayer(){
  ...
  socket.addEventListener('open', () => {
    sendMyState();
    const hb = setInterval(() => { if (socket?.readyState === WebSocket.OPEN) sendMyState(); else clearInterval(hb); }, 500);
  });
